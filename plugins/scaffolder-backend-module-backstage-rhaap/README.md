...
Note - This step is not required when testing the plugin with upstream backstage or downstream RHDH, because both have the `@backstage/plugin-scaffolder-backend-module-github` pre-registered.
For testing with midstream backstage-showcase, the step is required because
