...
## Installation - with midstream backstage-showcase

### Setup janus-idp backstage-showcase
...
Clone the backstage-showcase repository.
From the `dynamic-plugins-root`
folder run the below command

### Install and prepare the plugin

Install the frontend plugin dependency in the Ansible plugins path by running the following command from the `ansible-rhdh-plugins/plugins/backstage-rhaap-backend` folder.
...
To load the frontend plugin with backstage-showcase locally, follow the steps below.

- Run the following commands

```bash
pkg=<local-clone-parent-path-replace-me>/ansible-rhdh-plugins/plugins/backstage-rhaap
archive=$(npm pack $pkg)
tar -xzf "$archive" && rm "$archive"
mv package $(echo $archive | sed -e 's:\.tgz$::')
```

### Plugin registration with backstage-showcase
...
### Start the front end and backend by running the command in the root folder of `backstage-showcase` cloned repository path.
