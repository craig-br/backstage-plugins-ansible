...
```bash
docker pull quay.io/rhdh/rhdh-hub-rhel9:1.5
```

Note: If you're using an Apple Silicon (M1/M2) Mac, the default RHDH image (`quay.io/rhdh/rhdh-hub-rhel9:1.4`) is not compatible with the ARM64 architecture. Instead use the rhdh-community image:
...
```bash
git clone git@github.com:redhat-developer/rhdh.git
cd rhdh
...
1. Clone the rhdh-local repository to a location on your PC
...
1. Start RHDH Local.
   This repository should work with either `docker compose` using Docker Engine or `podman-compose` using Podman. When using Podman there are some exceptions.
